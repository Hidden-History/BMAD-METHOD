...
All patterns come from a **production BMAD Memory System** with validated metrics:
...
../_project/                        # Our workspace (NEVER commit)
  └── plans/
      └── streamed-sleeping-flurry.md     # Complete implementation plan
  └── session-notes/
      └── 2026-01-03-initial-setup.md     # Latest session handoff

../_reference/                      # Source materials (NEVER commit)
  └── guides/
      ├── BMAD_MEMORY_IMPLEMENTATION_EXPERIENCE.md  # 🔥 PRIMARY REFERENCE
      ├── bmad-qdrant-complete-guide.md
      ├── bmad-qdrant-monitoring-plan.md
      └── bmad-qdrant-memory-guide.md
```
...
**Benefit:** 92% faster comprehension (2-3 min → 10 sec)
...
```
BMAD-METHOD/
├── docker-compose.yml              # Qdrant + monitoring
├── .env.example
├── requirements.txt
│
├── src/core/
...
│   │   ├── client.py
│   │   ├── hooks/
│   │   │   ├── agent_hooks.py     # PROVEN pattern
│   │   │   ├── chat_hooks.py
│   │   │   ├── project_hooks.py
│   │   │   └── best_practices_hooks.py
│   │   ├── schemas/               # 8 JSON schemas
│   │   └── validation/
│   │
│   └── workflows/
│       └── tools/                  # NEW: Bridge scripts
│           ├── pre-work-search.py
│           ├── post-work-store.py
│           └── load-chat-context.py
│
├── scripts/
│   ├── memory-setup.sh            # Auto-setup
│   └── memory/
...
- Validate against BMAD Memory metrics (85% token savings, <1s search)
...
- Qdrant: 6333→16350, 6334→16351
- Prometheus: 9090→19095
- Grafana: 3000→13005
- Streamlit: 8501→18505
...
Must match or exceed BMAD Memory results:

| Metric | BMAD Memory | Target | Status |
|--------|----------|--------|--------|
| Token savings | 85% | ≥85% | Not measured |
| Search latency | <1s | <1s | Not implemented |
| Storage latency | <500ms | <500ms | Not implemented |
| Data quality | 100% | 100% | Not implemented |
| Cache efficiency | 99% | ≥95% | Not implemented |
...
### DO
✅ Apply all 10 proven patterns exactly as documented
✅ Reference BMAD_MEMORY_IMPLEMENTATION_EXPERIENCE.md frequently
✅ Test against BMAD Memory production metrics
✅ Keep code simple and focused
✅ Validate file:line references
✅ Enforce token budgets

### DON'T
❌ Skip any of the 10 proven patterns
❌ Commit secrets or environment files
❌ Commit `_project/` or `_reference/` directories
❌ Over-engineer solutions
❌ Add features not in the plan
❌ Deviate from proven patterns without justification
